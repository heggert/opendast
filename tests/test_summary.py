...
class TestPrintSummary(unittest.TestCase):
    def _capture_summary(self, findings, target, token_count):
        captured = io.StringIO()
        old_stdout = sys.stdout
        sys.stdout = captured
        try:
            print_summary(findings, target, token_count)
        finally:
            sys.stdout = old_stdout
        return captured.getvalue()

    def test_no_findings(self):
        output = self._capture_summary([], "http://example.com", "1,000/100,000")
